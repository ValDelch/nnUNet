...
# New for e2cnn ExperimentPlanner
from escnn import gspaces
from dynamic_network_architectures.architectures.unet_e2cnn import PlainConvUNet as PlainConvUNet_e2cnn
from dynamic_network_architectures.building_blocks.helper_e2cnn import convert_dim_to_conv_op as convert_dim_to_conv_op_e2cnn
from dynamic_network_architectures.building_blocks.helper_e2cnn import get_matching_batchnorm as get_matching_batchnorm_e2cnn
from nnunetv2.experiment_planning.experiment_planners.network_topology import get_pool_and_conv_props_e2cnn

# New for e3cnn ExperimentPlanner
from dynamic_network_architectures.architectures.unet_e3cnn import PlainConvUNet as PlainConvUNet_e3cnn

# New for BCNN ExperimentPlanner
...
class ExperimentPlannerE3CNN(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnUNetPlansE3CNN',
                 overwrite_target_spacing: Union[List[float], Tuple[float, ...]] = None,
                 suppress_transpose: bool = False):
        """
        overwrite_target_spacing only affects 3d_fullres! (but by extension 3d_lowres which starts with fullres may
    ...
    def get_plans_for_configuration(self,
        ...
        # pooling operations (must be divisible by 2**num_pool in each axis)
        network_num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, patch_size, \
        shape_must_be_divisible_by = get_pool_and_conv_props_e2cnn(spacing, initial_patch_size,
                                                             self.UNet_featuremap_min_edge_length,
                                                             999999)
        ...
        while estimate > reference:
            ...
            tmp[axis_to_be_reduced] -= shape_must_be_divisible_by[axis_to_be_reduced]
            _, _, _, _, shape_must_be_divisible_by = \
                get_pool_and_conv_props_e2cnn(spacing, tmp,
                                        self.UNet_featuremap_min_edge_length,
                                        999999)
            patch_size[axis_to_be_reduced] -= shape_must_be_divisible_by[axis_to_be_reduced]
            ...
            # now recompute topology
            network_num_pool_per_axis, pool_op_kernel_sizes, conv_kernel_sizes, patch_size, \
            shape_must_be_divisible_by = get_pool_and_conv_props_e2cnn(spacing, patch_size,
                                                                 self.UNet_featuremap_min_edge_length,
                                                                 999999)
...
class ExperimentPlannerE2CNN(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnUNetPlansE2CNN',
                 overwrite_target_spacing: Union[List[float], Tuple[float, ...]] = None,
                 suppress_transpose: bool = False):
        """
        overwrite_target_spacing only affects 3d_fullres! (but by extension 3d_lowres which starts with fullres may
    ...
    def static_estimate_VRAM_usage(patch_size: Tuple[int],
        ...
        """
        dim = len(patch_size)
        conv_op = convert_dim_to_conv_op_e2cnn(dim)
        norm_op = get_matching_batchnorm_e2cnn(conv_op)
        net = UNet_class(gspace,
                         num_input_channels, 
                         n_stages,
                         features_per_stage,
                         conv_op,
                         ...
                         blocks_per_stage_decoder,
                         norm_op=norm_op)
        return net.compute_conv_feature_map_size(patch_size, order)
...
class ExperimentPlannerBCNN(object):
    def __init__(self, dataset_name_or_id: Union[str, int],
                 gpu_memory_target_in_gb: float = 8,
                 preprocessor_name: str = 'DefaultPreprocessor', plans_name: str = 'nnUNetPlansBCNN',
                 overwrite_target_spacing: Union[List[float], Tuple[float, ...]] = None,
                 suppress_transpose: bool = False):
        """
        overwrite_target_spacing only affects 3d_fullres! (but by extension 3d_lowres which starts with fullres may
...
if __name__ == '__main__':
    ...
    ExperimentPlannerE2CNN(2, 8).plan_experiment()
    ExperimentPlannerBCNN(2, 8).plan_experiment()