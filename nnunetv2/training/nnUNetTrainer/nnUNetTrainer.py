...
from nnunetv2.training.loss.focal_loss import FL_and_CE_loss
...
# New for E2CNN
from nnunetv2.utilities.get_network_from_plans_e2cnn import get_network_from_plans as get_network_from_plans_e2cnn

# New for E3CNN
from nnunetv2.utilities.get_network_from_plans_e2cnn import get_network_from_plans as get_network_from_plans_e3cnn

# New for BCNN
from nnunetv2.utilities.get_network_from_plans_bcnn import get_network_from_plans as get_network_from_plans_bcnn
...
class nnUNetTrainer(object):
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = WarmupCosineScheduler(optimizer, self.initial_lr, self.num_epochs // 20, self.num_epochs)
...
class nnUNetTrainerE3CNN(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        """
        return get_network_from_plans_e3cnn(plans_manager, dataset_json, configuration_manager,
                                      num_input_channels, deep_supervision=enable_deep_supervision)
...
class nnUNetTrainerE3CNN_FL_CE(nnUNetTrainerE3CNN):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.loss = FL_and_CE_loss(alpha=0.5)
...
class nnUNetTrainerE3CNN_FL_CE_continue(nnUNetTrainerE3CNN_FL_CE):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.num_epochs = 500
...
class nnUNetTrainerE2CNN(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        """
        return get_network_from_plans_e2cnn(plans_manager, dataset_json, configuration_manager,
                                            num_input_channels, deep_supervision=enable_deep_supervision)
...
class nnUNetTrainerE2CNN_FL_CE(nnUNetTrainerE2CNN):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.loss = FL_and_CE_loss(alpha=0.5)
...
class nnUNetTrainerBCNN(nnUNetTrainer):
    ...
    def build_network_architecture(plans_manager: PlansManager,
        ...
        """
        return get_network_from_plans_bcnn(plans_manager, dataset_json, configuration_manager,
                                           num_input_channels, deep_supervision=enable_deep_supervision)
...
class nnUNetTrainerBCNN_FL_CE(nnUNetTrainerBCNN):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
                 device: torch.device = torch.device('cuda')):
        super().__init__(plans, configuration, fold, dataset_json, unpack_dataset, device)
        self.loss = FL_and_CE_loss(alpha=0.5)
